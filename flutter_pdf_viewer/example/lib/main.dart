...
class _ExampleApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(home: _ExampleScreen());
  }
}
...
class _ExampleScreenState extends State<_ExampleScreen> {
  ...
  String _activePdfLabel = 'blank_a4_portrait_1p.pdf';
  ...
  void initState() {
    super.initState();
    _createInitialBlankPdf();
  }

  Future<void> _createInitialBlankPdf() async {
    final bytes = await _documentCreator.createBlankPdf(
    ...
    await _setActiveDocument(
      ...
      label: 'blank_a4_portrait_1p.pdf',
  ...
  Future<void> _exportEditablePdf() async {
    if (_editorMode == _EditorMode.authoring) {
      setState(() {
        _exportMessage =
            'Editable export is only available for opened AcroForm PDFs. Blank documents export as flattened PDFs.';
      });
      return;
    }
    if (_isExporting) {
    ...
    try {
      ...
      setState(() {
        _exportMessage = 'Editable export failed: ${error.message}';
      });
    } catch (error) {
      if (!mounted) {
        return;
  ...
  Future<void> _savePdfToUserPath() async {
    try {
      ...
      setState(() {
        _exportMessage =
            'Saved PDF to $destinationPath (${exportedBytes.length} bytes)';
      });
    } catch (error) {
      if (!mounted) {
        return;
  ...
  Future<void> _shareExportedPdf() async {
    try {
      ...
      setState(() {
        _exportMessage = 'Shared PDF from ${exportedFile.path}';
      });
    } catch (error) {
      if (!mounted) {
        return;
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              isAuthoringMode
                  ? 'Create a blank PDF, add overlays, then export the result.'
                  : 'Open a PDF from file, edit AcroForm fields, then export flattened or editable results.',
